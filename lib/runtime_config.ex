defmodule Bonfire.XMPP.RuntimeConfig do
  ...
  def config do
    ...
    conf_root = release_path || case System.get_env("RELIVE", "false") do
      "true" -> "_build/relive"
    ...
    config :ejabberd,
      ...
      log_path: System.get_env("EJABBERD_LOG") || Path.join(release_path || "", "data/xmpp/logs/ejabberd.log")
  ...
  defp substitute_ejabberd_config(conf_root, hostname, is_dev) do
    ...
    case File.read(template_path) do
      {:ok, content} ->
        ...
        |> String.replace("localhost", hostname)  # Replace any remaining localhost references
